...
<head>
    ...
    <title>@yield('title', 'Portal RH - SorriDoc')</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
        .bg-sorridoc-600 { background-color: #0f766e; }
        .text-sorridoc-600 { color: #0f766e; }
        .bg-sorridoc-50 { background-color: #f0fdfa; }
        .border-sorridoc-200 { border-color: #99f6e4; }
    </style>
</head>
            ...
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    <div class="flex-shrink-0 flex items-center">
                        ...
                        <span class="ml-2 text-sm text-teal-200">| Portal RH</span>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-sm">{{ $company->corporate_name }}</span>
                    <form method="POST" action="{{ route('rh.logout', $token) }}">
                        @csrf
