    ...
    <div class="flex items-center w-full max-w-2xl mx-auto space-x-4">
        ...
        <div class="flex flex-col items-center">
            ...
            <span class="mt-2 text-xs font-semibold {{ $step1 ? 'text-primary-700' : 'text-gray-500' }}">
                {{ $step1 ? $step1->assignedTo->name : ($isDupla ? 'Diretor' : 'Assinatura') }}
            </span>
            @if($step1)
        ...
        @if($isDupla)
            ...
            <div class="flex flex-col items-center">
                ...
                <span class="mt-2 text-xs font-semibold {{ $step2 ? 'text-primary-700' : 'text-gray-500' }}">
                    {{ $step2 ? $step2->assignedTo->name : 'Advogada' }}
                </span>
                @if($step2)
        ...
        <!-- Final -->
        <div class="flex flex-col items-center">
            <div class="flex items-center justify-center w-10 h-10 border-2 {{ $status === 'Aprovado' ? 'border-green-600 bg-green-50' : 'border-gray-300' }} rounded-full transition-all duration-300">
                <svg class="w-6 h-6 {{ $status === 'Aprovado' ? 'text-green-600' : 'text-gray-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <span class="mt-2 text-xs font-bold {{ $status === 'Aprovado' ? 'text-green-700' : 'text-gray-500' }}">
                {{ $status === 'Aprovado' ? 'Certificado' : ($isRejeitado ? 'Bloqueado' : 'Finalizar') }}
            </span>
        </div>
    </div>
</div>
